...
"#,
    ...
    let graph = syntra::graph::NeuralGraph::from_bytes(&bytes).expect("valid lyc graph");
    assert!(
        graph
            .nodes
            .iter()
            .any(|n| matches!(n.op, syntra::graph::OpCode::AdaptiveChoice))
    );
...
#[test]
fn proof_lab_erdos190_reports_h3_and_all_six_components() {
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_syntra"))
        .arg("proof-lab")
        .arg("erdos190")
        .arg("--k")
    ...
    assert_eq!(report["exact_h"], 9);
    assert_eq!(report["components"].as_array().unwrap().len(), 6);
    assert!(
        report["lean_skeleton"]
            .as_str()
            .unwrap()
    ...
    assert!(
        report["warnings"]
            ...
            .any(|warning| warning
                .as_str()
                .unwrap()
                .contains("does not claim a machine-checked proof"))
    );
}

#[test]
fn proof_lab_writes_markdown_report_and_lean_skeleton() {
    let root = std::env::temp_dir().join(format!(
        "syntra-proof-lab-{}-{}",
        std::process::id(),
        unique_suffix()
    ...
    let report_path = root.join("erdos190.md");
    let lean_path = root.join("erdos190.lean");
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_syntra"))
        .arg("proof-lab")
        .arg("erdos190")
        .arg("--k")
    ...
    let report = std::fs::read_to_string(&report_path).expect("read markdown report");
    let lean = std::fs::read_to_string(&lean_path).expect("read lean skeleton");
    assert!(report.contains("# Syntra Proof Lab Report"), "{report}");
    assert!(report.contains("## Proof Obligations"), "{report}");
    assert!(lean.contains("finite_exhaustion_obligation"), "{lean}");
...
fn proof_lab_erdos160_reports_verified_h_values_and_refuses_asymptotic() {
    ...
    assert!(
        report["warnings"]
            ...
            .unwrap()
            .iter()
            .any(|w| w.as_str().unwrap().contains("does not resolve Erdos #160"))
    );
}
...
fn proof_lab_erdos160_sat_backend_reports_certificates_patterns_and_bounds() {
    ...
    assert!(report["certificates"].as_array().unwrap().iter().any(
        |certificate| certificate["kind"] == "exhaustive_unsat"
...
mod e2e {
    ...
    fn full_round_trip_author_install_decide_feedback_components() {
        ...
        std::thread::spawn(move || {
            syntra::server::run_server(syntra::server::ServerConfig {
                addr: server_addr,
...
"#,
        ...
        assert!(
            decision.get("decisionId").is_some(),
            "decide must return decisionId: {decision}"
        );
...
"#,
        ...
        assert_eq!(
            s,
            400,
            "feedback without rewardSpec should 400; body={}",
            String::from_utf8_lossy(&body)
        );
...
"#,
        ...
        assert_eq!(
            initial["warmup"]["state"], "warmup",
            "initial decide should report warmup state"
        );
        ...
        for i in 1..=30 {
            ...
            if fb["warmupTransitioned"].as_bool().unwrap_or(false) {
                ...
                assert_eq!(
                    fb["warmup"]["state"], "active",
                    "transition should report active state"
                );
            }
        }
        assert_eq!(
            transitioned_round,
            Some(30),
            "warmup should transition on the 30th feedback"
        );
        ...
        assert_eq!(
            after["warmup"]["state"], "active",
            "post-warmup decide should report active"
        );
        let algo = after["warmup"]["algorithm"].as_str().unwrap_or("");
        assert!(
            algo.contains("Thompson"),
            "binary rewards should pick Thompson, got: {algo}"
        );

        let warmup_path = store_root
            .join("tenants")
            .join(tenant)
            .join("jobs")
            .join(job)
        ...
        assert!(
            warmup_path.exists(),
            "warmup.json should be persisted in capsule dir"
        );
...
"#,
    ...
    fn meta_bandit_active_after_warmup() {
        ...
        let any_with_meta = strategies
            .values()
            .any(|s| s.get("metaBandit").map(|v| !v.is_null()).unwrap_or(false));
        let any_with_cand_ctx = strategies.values().any(|s| {
            s.get("candidateContexts")
        ...
        assert!(
            any_with_meta,
            "memory.json must contain a metaBandit after Active feedback"
        );
        assert!(
            any_with_cand_ctx,
            "memory.json must contain candidateContexts after Active feedback"
        );
    ...
    fn change_detection_resets_meta_bandit() {
        ...
        for i in 0..400 {
            ...
            assert_eq!(s, 200);
            if json_body(&body)["changeDetected"]
                .as_bool()
                .unwrap_or(false)
            {
                change_round = Some(i);
        ...
        assert!(
            change_round.is_some(),
            "change should be detected after regime shift"
        );
    ...
    fn per_context_change_detection_resets_only_drifted_context() {
        ...
        let mem_before: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&mem_path).unwrap()).unwrap();
        let cands_before: Vec<String> = mem_before["strategies"]
            .as_object()
            .unwrap()
            .values()
            ...
            .collect();
        assert!(
            cands_before.iter().any(|k| k.ends_with("|tier_a")),
            "tier_a should have candidate state before drift; got: {cands_before:?}"
        );
        assert!(
            cands_before.iter().any(|k| k.ends_with("|tier_b")),
        ...
        for i in 0..500 {
            ...
            let (_, body) = http(
                ...
                &decide_path,
                Some(admin_key),
                br#"{"contextKey":"tier_a","inputs":{"tier":"tier_a"}}"#,
                "application/json",
            );
            let did = json_body(&body)["decisionId"].as_str().unwrap().to_string();
            let r = if (i * 7919) % 10 < 9 { 1.0 } else { 0.0 };
            ...
            let (_, body) = http(
                ...
                &decide_path,
                Some(admin_key),
                br#"{"contextKey":"tier_b","inputs":{"tier":"tier_b"}}"#,
                "application/json",
            );
            let did = json_body(&body)["decisionId"].as_str().unwrap().to_string();
            let r = if (i * 7919) % 10 < 1 { 1.0 } else { 0.0 };
            ...
            {
                panic!(
                    "capsule-level fired during interleaved drift at iteration {i}; the test premise needs tuning"
                );
            }
        }
        assert!(
            fired,
            "per-context change detector should fire on tier_a drift while capsule stays Active"
        );
    ...
    fn long_horizon_run_keeps_option_state_bounded_under_forgetting() {
        ...
        assert!(
            max_alpha < 600.0,
            "Beta α exceeded forgetting ceiling: {max_alpha}"
        );
        assert!(
            max_beta < 600.0,
            "Beta β exceeded forgetting ceiling: {max_beta}"
        );
        assert!(
            max_ucb_tries < 1500.0,
            "UCB tries didn't decay: {max_ucb_tries}"
        );
...
"#,
    ...
    fn linucb_capsule_routes_and_learns() {
            ...
            if let Some(first) = decisions.first() {
                if first["candidateId"].as_str() == Some("LinUcb") {
                    ...
                    if linucb_decision_id.is_none() {
                        ...
                        assert!(
                            first["featureVector"].as_array().is_some(),
        ...
        assert!(
            linucb_chosen > 0,
            "LinUcb should be selected at least once over 200 decides"
        );
        ...
        assert!(
            found_linucb_state,
            "memory must contain LinUcb option_states"
        );
        assert_eq!(
            meta_candidate_count, 7,
            "feature-context capsule must register 7 candidates (5 discrete + LinUcb + LinTs)"
        );
    ...
    fn linucb_capsule_rejects_invalid_features() {
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"features":{"age":30.0}}"#,
            "application/json",
        );
        assert_eq!(
            s,
            400,
            "missing feature should 400; got body={}",
            String::from_utf8_lossy(&body)
        );
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"features":{"age":30.0,"tier":99}}"#,
            "application/json",
        );
        assert_eq!(
            s,
            400,
            "type mismatch should 400; got body={}",
            String::from_utf8_lossy(&body)
        );
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"features":{"age":30.0,"tier":"platinum"}}"#,
            "application/json",
        );
        assert_eq!(
            s,
            400,
            "unknown category should 400; got body={}",
            String::from_utf8_lossy(&body)
        );
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"contextKey":"some_key"}"#,
            "application/json",
        );
        assert_eq!(
            s,
            400,
            "feature-context capsule should reject discrete contextKey; got body={}",
            String::from_utf8_lossy(&body)
        );
        ...
        let (s, _) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"features":{"age":30.0,"tier":"gold"}}"#,
            "application/json",
        );
        assert_eq!(s, 200);
    ...
    fn discrete_capsule_ood_score_appears_in_response() {
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"contextKey":"known","inputs":{"tier":"gold"}}"#,
            "application/json",
        );
        assert_eq!(s, 200);
        let resp = json_body(&body);
        ...
        assert!(
            known_score < 0.1,
            "known key oodScore = {known_score} (expected < 0.1)"
        );
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"contextKey":"never_seen","inputs":{"tier":"gold"}}"#,
            "application/json",
        );
        assert_eq!(s, 200);
        let resp = json_body(&body);
    ...
    fn set_refusal_config(
        addr: &str,
        admin_key: &str,
        tenant: &str,
        job: &str,
        capsule: &str,
        refusal_json: serde_json::Value,
        context_spec: Option<serde_json::Value>,
    ) {
    ...
    fn refusal_disabled_capsule_never_refuses() {
        ...
        assert!(
            resp["decisions"].as_array().unwrap().len() > 0,
            "decisions array should be non-empty"
        );
        let conf = &resp["confidence"];
        assert!(
            (conf["oodScore"].as_f64().unwrap() - 1.0).abs() < 1e-9,
    ...
    fn refusal_enabled_capsule_refuses_on_ood() {
        ...
        assert!(
            audit_text.contains("decision_refused"),
    ...
    fn refusal_enabled_capsule_refuses_on_wide_interval() {
        ...
        let resp = json_body(&body);
        assert_eq!(
            resp["refused"].as_bool(),
            Some(true),
            "expected refused; got {resp}"
        );
        assert_eq!(
            resp["confidence"]["refusalReason"].as_str(),
            Some("interval_too_wide")
        );
    ...
    fn feedback_on_refused_decision_only_updates_ood() {
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"contextKey":"brand_new","inputs":{"tier":"gold"}}"#,
            "application/json",
        );
        assert_eq!(s, 200);
        ...
        assert!(
            fb_resp["noted"].as_str().unwrap().contains("refused"),
    ...
    fn feature_capsule_ood_score_appears_in_response() {
        ...
        let (s, body) = http(
            ...
            &decide_path,
            Some(admin_key),
            br#"{"features":{"age":99.0,"tier":"free"}}"#,
            "application/json",
        );
        assert_eq!(s, 200);
    ...
    fn install_with_strategy_node_succeeds_and_is_non_blocking() {
        ...
        let program = syntra::parser::Parser::new(tokens)
            .parse_program()
            .expect("parse");
        let graph = syntra::graph_compiler::GraphCompiler::new()
            .compile(&program)
            .unwrap();
        let strategy_count = graph
            .nodes
            .iter()
            .filter(|n| matches!(n.op, syntra::graph::OpCode::Strategy))
            .count();
        assert!(
            strategy_count > 0,
        ...
        assert!(
            ...
            "warning must NOT be in the HTTP response"
        );
        ...
        let stored_graph =
            syntra::graph::NeuralGraph::from_bytes(&stored_bytes).expect("valid stored .lyc");
        assert!(
            stored_graph
                .nodes
                .iter()
                .any(|n| matches!(n.op, syntra::graph::OpCode::Strategy)),
    ...
    fn decision_log_rotation_keeps_api_stream_continuous() {
        ...
        std::fs::create_dir_all(&store_root).unwrap();
        std::fs::write(
            store_root.join("retention.json"),
            r#"{"maxLogBytes": 2048}"#,
        )
        .unwrap();
        let addr = spawn_server(&store_root, admin_key);
        ...
        let cap_dir = store_root
            ...
            .join(capsule);
        assert!(
            cap_dir.join("decision.jsonl.1").exists(),
            "40 decisions past a 2KiB cap must have rotated the log"
        );
        assert!(
            cap_dir.join("decision.jsonl").metadata().unwrap().len() <= 2048,
        ...
        assert!(
            served.len() < ids.len(),
            "retention must bound the stream: served {} of {}",
            served.len(),
            ids.len()
        );
        assert!(
            ids.ends_with(served.as_slice()),
        ...
        assert_eq!(
            s, 200,
            "second-newest decision must be creditable across rotation"
        );
        ...
        assert_eq!(
            s, 404,
            "decision dropped by retention must 404, never credit blindly"
        );
