...
use std::fs;
use syntra::change_detection::AdwinDetector;
...
fn characterize_adwin_delta_grid() {
    ...
    let drift_first_fire: Vec<Option<usize>> = deltas
        .iter()
        .map(|d| first_fire_index(*d, &drift))
        .collect();
    let control_fire_counts: Vec<usize> = deltas.iter().map(|d| fire_count(*d, &control)).collect();
    ...
    writeln!(
        md,
        "Generated by `cargo test --test change_detection_characterization`."
    )
    .unwrap();
    writeln!(
        md,
        "Synthetic data; not a production calibration. See known-issues.md."
    )
    .unwrap();
    writeln!(md).unwrap();
    ...
    writeln!(
        md,
        "- Drift: 100 samples from N(0.2, 0.1), then 100 from N(0.8, 0.1) (LCG seed `{:#x}`).",
        DRIFT_SEED
    )
    .unwrap();
    writeln!(
        md,
        "- Control: 200 samples from N(0.5, 0.1) (LCG seed `{:#x}`).",
        CONTROL_SEED
    )
    .unwrap();
    writeln!(md).unwrap();
    writeln!(
        md,
        "**Per-delta single-detector first-fire index on drift stream**"
    )
    .unwrap();
    writeln!(md).unwrap();
    ...
    writeln!(
        md,
        "Cell: `C` = capsule fired first; `X` = context fired first; "
    )
    .unwrap();
    ...
    writeln!(
        md,
        "Each cell shows `capsule_fires / context_fires` across 200"
    )
    .unwrap();
    writeln!(
        md,
        "stationary samples. Ideally both are 0; the prompt's bar is"
    )
    .unwrap();
    ...
    writeln!(
        md,
        "**Per-delta false-positive count over 200 stationary samples**"
    )
    .unwrap();
    writeln!(md).unwrap();
    ...
    writeln!(
        md,
        "Rationale. We want context to fire first on narrow drift, so"
    )
    .unwrap();
    writeln!(
        md,
        "context delta must be LOOSER (larger) than capsule delta —"
    )
    .unwrap();
    writeln!(
        md,
        "smaller delta = wider Hoeffding bound = slower to fire. The"
    )
    .unwrap();
    writeln!(md, "pair `(capsule=0.0005, context=0.002)` sits in the `X`").unwrap();
    writeln!(
        md,
        "(context-first) region of the matrix on the synthetic drift"
    )
    .unwrap();
    writeln!(
        md,
        "stream and posts 0 false positives on the stationary control"
    )
    .unwrap();
    ...
    writeln!(
        md,
        "The prior default `(capsule=0.0001, context=0.002)` was also"
    )
    .unwrap();
    writeln!(
        md,
        "ordered the right way, but the capsule-level detector fired"
    )
    .unwrap();
    writeln!(
        md,
        "~7 samples later than the chosen one (127 vs 124 above) and"
    )
    .unwrap();
    writeln!(
        md,
        "the gap widens for more subtle shifts; loosening capsule to"
    )
    .unwrap();
    writeln!(
        md,
        "0.0005 keeps a meaningful sample-count buffer between context"
    )
    .unwrap();
    writeln!(
        md,
        "and capsule firing without making capsule a near-tie with"
    )
    .unwrap();
    writeln!(
        md,
        "context. Larger capsule deltas (0.001+) close the buffer or"
    )
    .unwrap();
    ...
    writeln!(
        md,
        "These defaults are synthetic. Real workloads almost certainly"
    )
    .unwrap();
    writeln!(
        md,
        "have non-Gaussian reward distributions, heavier tails, and"
    )
    .unwrap();
    writeln!(
        md,
        "more gradual shifts. If on a stable workload you observe"
    )
    .unwrap();
    writeln!(
        md,
        "capsule-level firing before per-context, tighten capsule_delta"
    )
    .unwrap();
