...
fn healthy_store(label: &str) -> PathBuf {
    ...
    store.install_capsule_bytes("acme", "cap1", MAB_LYC).unwrap();
    root
}

fn capsule_dir(root: &Path) -> PathBuf {
    root.join("tenants").join("acme").join("jobs").join("default")
        .join("capsules").join("cap1")
}
...
fn find_code(stdout: &str, code: &str) -> Option<serde_json::Value> {
    stdout.lines().filter_map(|l| serde_json::from_str::<serde_json::Value>(l).ok())
...
fn doctor_healthy_store_exits_zero() {
    ...
    assert!(stdout.lines().any(|l| l.contains("\"type\":\"summary\"")), "summary line required:\n{stdout}");
...
fn doctor_reports_corrupt_memory() {
    ...
    std::fs::write(capsule_dir(&root).join("memory.json"), b"{\"version\": 7, \"strate").unwrap();
    let (code, stdout, _) = doctor(&root, &[]);
    assert_eq!(code, 1);
...
fn corrupt_load_paths_log_and_preserve_evidence_before_reset() {
    ...
    let copies: Vec<_> = std::fs::read_dir(&cap).unwrap().flatten()
        .filter(|e| e.file_name().to_string_lossy().starts_with("memory.json.corrupt-"))
        .collect();
    assert_eq!(copies.len(), 1, "exactly one evidence copy after first corrupt read");
    assert_eq!(std::fs::read(copies[0].path()).unwrap(), b"NOT JSON AT ALL {{{",
        "evidence must be a byte-for-byte copy of the corrupt file");

    // Repeat resets must not accumulate evidence files.
    assert!(store.load_memory_in_job("acme", "default", "cap1").is_err());
    let copies_after: Vec<_> = std::fs::read_dir(&cap).unwrap().flatten()
        .filter(|e| e.file_name().to_string_lossy().starts_with("memory.json.corrupt-"))
        .collect();
    assert_eq!(copies_after.len(), 1, "evidence is bounded to one copy per name");
    ...
    assert!(store.load_hierarchical_state_in_job("acme", "default", "cap1").is_none());
    assert!(std::fs::read_dir(&cap).unwrap().flatten()
        .any(|e| e.file_name().to_string_lossy().starts_with("hierarchical_state.json.corrupt-")),
...
fn doctor_reports_orphan_tmp_files() {
    ...
    let (code, stdout, _) = doctor(&root, &[]);
    assert_eq!(code, 1);
    let hits: Vec<_> = stdout.lines().filter(|l| l.contains("TMP_ORPHAN")).collect();
    assert_eq!(hits.len(), 2, "both tmp naming schemes must be flagged:\n{stdout}");
    let _ = std::fs::remove_dir_all(&root);
}
...
fn doctor_detects_torn_jsonl_tail() {
    ...
    let cap = capsule_dir(&root);
    std::fs::write(cap.join("decision.jsonl"),
        b"{\"id\":\"dec_1\",\"ok\":true}\n{\"id\":\"dec_2\",\"part").unwrap();
    let (code, stdout, _) = doctor(&root, &[]);
    assert_eq!(code, 1);
...
fn doctor_detects_stale_evolve_lock() {
    ...
    std::fs::write(capsule_dir(&root).join(".evolve.lock"), std::process::id().to_string()).unwrap();
    let (code, stdout, _) = doctor(&root, &[]);
...
fn doctor_warns_on_memory_version_drift() {
    ...
    std::fs::write(capsule_dir(&root).join("memory.json"),
        r#"{"version":3,"strategies":{}}"#).unwrap();
    let (code, stdout, _) = doctor(&root, &[]);
...
fn doctor_json_flag_emits_findings_only() {
    ...
    for line in stdout.lines() {
        ...
        assert_ne!(j["type"].as_str(), Some("summary"), "--json must omit the summary line");
    }
...
fn syntra_json(args: &[&str]) -> (i32, String, String) {
    let out = Command::new(env!("CARGO_BIN_EXE_syntra"))
        .args(args).output().expect("run syntra");
    (out.status.code().unwrap_or(-1),
     String::from_utf8_lossy(&out.stdout).to_string(),
     String::from_utf8_lossy(&out.stderr).to_string())
}
...
fn backup_restore_round_trip_recovers_a_live_store() {
    ...
    let spawn = || Command::new(env!("CARGO_BIN_EXE_syntra"))
        .args(["serve", "--addr", &addr, "--store"]).arg(&root)
    ...
    auth(ureq::post(&format!("{base}/install")))
        ...
        .send_bytes(MAB_LYC).expect("install");
    let d = auth(ureq::post(&format!("{base}/decide?learn=true")))
        .set("Content-Type", "application/json")
        .send_string(r#"{"inputs":{"x":1}}"#).expect("decide")
    ...
    let (code, out, err) = syntra_json(&["backup", "--store", root.to_str().unwrap(),
        "--out", bundle.to_str().unwrap()]);
    assert_eq!(code, 0, "backup failed: {err}");
    let j: serde_json::Value = serde_json::from_str(out.trim()).unwrap();
    assert_eq!(j["ok"], true);
    assert!(j["files"].as_u64().unwrap() > 5, "bundle should hold the store: {j}");
    let body = std::fs::read(&bundle).unwrap();
    assert_eq!(serde_json::from_slice::<serde_json::Value>(&body).unwrap()["v"], 1);
    ...
    let (code, out, err) = syntra_json(&["restore", "--bundle", bundle.to_str().unwrap(),
        "--into", root.to_str().unwrap()]);
    ...
    let d = auth(ureq::post(&format!("{base}/decide?learn=true")))
        .set("Content-Type", "application/json")
        .send_string(r#"{"inputs":{"x":1}}"#).expect("decide after restore")
        .into_json::<serde_json::Value>().unwrap();
...
fn restore_refuses_a_live_root_without_force() {
    ...
    let (code, _, err) = syntra_json(&["backup", "--store", root.to_str().unwrap(),
        "--out", bundle.to_str().unwrap()]);
    assert_eq!(code, 0, "{err}");

    // Evidence 1: readiness probe present => serving (or crashed mid-/ready).
    std::fs::write(root.join(".readiness_probe"), b"").unwrap();
    let (code, _, err) = syntra_json(&["restore", "--bundle", bundle.to_str().unwrap(),
        "--into", root.to_str().unwrap()]);
    ...
    std::fs::write(capsule_dir(&root).join(".evolve.lock"),
