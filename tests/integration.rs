...
fn test_runtime_ephemeris_capability_source_and_binary() {
    let code = r#"
        ($ eph "examples/lycan/data/apophis_spice_nav.lye")
        ($ body "20099942")
...
fn test_proposal_impure_rejected() {
    ...
    std::fs::copy("examples/lycan/demo_impossible.lyc", &lyc).ok();
...
fn test_edge_of_chaos_binary_runs() {
    // Run a COPY: executing a .lyc persists learned weights back into the
    // source (enforced by tests/fixture_drift.rs).
    let tmp = std::env::temp_dir().join(format!("syntra-eoc-{}.lyc", std::process::id()));
...
fn test_control_chaos_feedback_selects_edge_controller() {
    ...
    std::fs::copy("examples/lycan/demo_control_chaos.lycs", &src).unwrap();
...
fn test_planetary_defense_feedback_selects_hybrid() {
    ...
    std::fs::copy("examples/lycan/demo_planetary_defense.lycs", &src).unwrap();
...
fn test_horizons_apophis_feedback_selects_rk4() {
    ...
    std::fs::copy("examples/lycan/demo_horizons_apophis.lycs", &src).unwrap();
...
fn test_horizons_apophis_full_feedback_selects_full_model() {
    ...
    std::fs::copy("examples/lycan/demo_horizons_apophis_full.lycs", &src).unwrap();
...
fn test_spice_nav_apophis_feedback_selects_sub_10cm_model() {
    ...
    std::fs::copy("examples/lycan/demo_spice_nav_apophis.lycs", &src).unwrap();
...
fn test_antiviral_feedback_selects_best_target_classes() {
    ...
    std::fs::copy("examples/lycan/demo_antiviral_target_selection.lycs", &src).unwrap();
...
decision_demo_test!(
    ...
    "examples/lycan/demo_spacecraft_fault_manager.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_pandemic_policy.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_icu_triage.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_grid_blackout_prevention.lycs",
    "Best grid policy: 4",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_flood_response.lycs",
    "Best flood policy: 3",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_fraud_chargeback.lycs",
    "Best fraud policy: 2",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_takeaway_demand.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_takeaway_chaos_replay.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_cyber_triage.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_compiler_optimizer.lycs",
...
decision_demo_test!(
    ...
    "examples/lycan/demo_query_planner.lycs",
