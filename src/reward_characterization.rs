...
pub fn characterize(rewards: &[f64]) -> RewardShape {
    ...
    let all_binary = rewards
        .iter()
        .all(|r| r.abs() < BINARY_TOL || (r - 1.0).abs() < BINARY_TOL);
    if all_binary {
        let pos = rewards.iter().filter(|r| **r > 0.5).count() as f64;
        return RewardShape::Binary {
            positive_rate: pos / rewards.len() as f64,
        };
    }
    ...
    if zero_ratio > SPARSITY_THRESHOLD {
        let nz: Vec<f64> = rewards
            .iter()
            .filter(|r| r.abs() >= BINARY_TOL)
            .copied()
            .collect();
        if nz.is_empty() {
            return RewardShape::Unknown {
                reason: "all zero".to_string(),
            };
        }
...
pub fn pick_algorithm(shape: &RewardShape) -> PickedAlgorithm {
    match shape {
        RewardShape::Binary { .. } => PickedAlgorithm::Thompson {
            alpha: 1.0,
            beta: 1.0,
        },
        ...
        RewardShape::Unknown { .. } => PickedAlgorithm::Weighted {
            learning_rate: 0.05,
        },
    }
}
