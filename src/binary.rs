...
/// `Vec::with_capacity` — mirrors the DoS guards in `graph.rs`. Without
/// this, a tiny hostile `.lyc` can request a u32::MAX-element allocation
/// and OOM-kill the process.
...
fn read_str(data: &[u8], pos: &mut usize) -> String {
    ...
    let len = len.min(data.len().saturating_sub(*pos));
