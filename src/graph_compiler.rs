use crate::ast::*;
use crate::graph::*;
use crate::graph::{Contract, WeightKind};
...
impl GraphCompiler {
    ...
    fn compile_node(&mut self, node: &Node) -> u32 {
        match node {
            Node::Int(n) => self.graph.add_node(
                OpCode::ConstInt,
                vec![Operand::Immediate(ImmValue::Int(*n))],
            ),
            Node::Float(f) => self.graph.add_node(
                OpCode::ConstFloat,
                vec![Operand::Immediate(ImmValue::Float(*f))],
            ),
            Node::Str(s) => {
                let idx = self.graph.intern_string(s);
                self.graph
                    .add_node(OpCode::ConstStr, vec![Operand::StringRef(idx)])
            }
            Node::Bool(b) => self.graph.add_node(
                OpCode::ConstBool,
                vec![Operand::Immediate(ImmValue::Bool(*b))],
            ),
            Node::Null => self
                .graph
                .add_node(OpCode::ConstNull, vec![Operand::Immediate(ImmValue::Null)]),

            Node::Ident(name) => {
                let slot = self.get_or_create_var(name);
                self.graph
                    .add_node(OpCode::LoadVar, vec![Operand::VarSlot(slot)])
            }
            ...
            Node::Bind { name, value, .. } => {
                ...
                self.graph.add_node(
                    OpCode::StoreVar,
                    vec![Operand::VarSlot(slot), Operand::NodeRef(val_id)],
                )
            }
            ...
            Node::Op { op, args } => {
                let opcode = match op {
                    ...
                    OpKind::Eq => OpCode::Eq,
                    OpKind::Neq => OpCode::Neq,
                    OpKind::Lt => OpCode::Lt,
                    OpKind::Gt => OpCode::Gt,
                    OpKind::Lte => OpCode::Lte,
                    OpKind::Gte => OpCode::Gte,
            ...
            Node::ForEach {
                ...
                let body_ids: Vec<Operand> = body
                    ...
                    .map(|n| Operand::NodeRef(self.compile_node(n)))
                    .collect();
                let mut operands = vec![Operand::NodeRef(iter_id), Operand::VarSlot(var_slot)];
                operands.extend(body_ids);
            ...
            Node::Fn {
                ...
                if let Some(fn_name) = name {
                    ...
                    let store = self.graph.add_node(
                        OpCode::StoreVar,
                        vec![Operand::VarSlot(slot), Operand::NodeRef(fn_node)],
                    );
                    return store;
                }
                fn_node
            ...
            Node::Return(val) => {
                ...
                self.graph
                    .add_node(OpCode::Return, vec![Operand::NodeRef(val_id)])
            }

            Node::Block(exprs) => {
                let operands: Vec<Operand> = exprs
                    .iter()
                    .map(|n| Operand::NodeRef(self.compile_node(n)))
                    .collect();
            ...
            Node::Array(elems) => {
                let operands: Vec<Operand> = elems
                    .iter()
                    .map(|n| Operand::NodeRef(self.compile_node(n)))
                    .collect();
            ...
            Node::Index { object, index } => {
                ...
                self.graph.add_node(
                    OpCode::Index,
                    vec![Operand::NodeRef(obj_id), Operand::NodeRef(idx_id)],
                )
            }
            ...
            Node::Range { start, end } => {
                ...
                self.graph.add_node(
                    OpCode::Range,
                    vec![Operand::NodeRef(s), Operand::NodeRef(e)],
                )
            }
            ...
            Node::Pipe {
                ...
                let mut operands = vec![Operand::NodeRef(data_id), Operand::NodeRef(func_id)];
            ...
            Node::Choice { options } => {
                let operands: Vec<Operand> = options
                    .iter()
                    .map(|n| Operand::NodeRef(self.compile_node(n)))
                    .collect();
                let n = operands.len();
            ...
            Node::Feedback { target, reward } => {
                // Resolve target: if it's a variable, find the original graph
                // node. Fail-closed: an unresolvable name (typo, or a variable
                // assigned with `=` instead of bound with `$`) used to compile
                // to a bare LoadVar reference whose credit the runtime silently
                // dropped; a name bound to a non-choice value updated nothing.
                // Both now refuse to compile — Feedback targets
                // AdaptiveChoice/Strategy nodes only (learning-semantics §4.2).
                let t = match target.as_ref() {
                    Node::Ident(name) => match self.var_node.get(name).copied() {
                        Some(id) => {
                            if !self.graph.nodes.get(id as usize).is_some_and(|n| {
                                matches!(n.op, OpCode::AdaptiveChoice | OpCode::Strategy)
                            }) {
                                self.errors.push(format!(
                                    "feedback target '{name}' is bound to a non-choice value; feedback requires ($ {name} (choice ...))"
                                ));
                            }
                            id
                        }
                        None => {
                            self.errors.push(format!(
                                "feedback target '{name}' is not bound; bind the choice with ($ {name} (choice ...))"
                            ));
                ...
                self.graph.add_node(
                    OpCode::Feedback,
                    vec![Operand::NodeRef(t), Operand::NodeRef(r)],
                )
            }
