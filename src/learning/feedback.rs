...
fn check_change_point_page_hinkley(
    s: &mut OptionStats,
    reward: f64,
    ...
    let mean = if s.effective_tries > 1.0 {
        s.reward_sum / s.effective_tries
    } else {
        s.reward_mean()
    };
...
fn check_change_point_model_surprise(
    s: &mut OptionStats,
    reward: f64,
    config: &ChangeDetectionConfig,
) -> bool {
    if s.tries < 5 || s.window.len() < 4 {
        return false;
    }
    ...
    if surprising {
        s.surprise_recent = s.surprise_recent.saturating_add(1);
    }
...
fn check_change_point(s: &mut OptionStats, reward: f64, config: &ChangeDetectionConfig) -> bool {
    ...
    match config.method {
        ...
        ChangeDetectionMethod::ModelSurprise => {
            check_change_point_model_surprise(s, reward, config)
        }
    }
}
...
pub fn compute_prediction_set(
    ...
    let scores: Vec<f64> = (0..n_options)
        .map(|i| {
            bucket
                .stats
                .get(i)
                .map(|s| stats_score(s, config))
...
pub fn apply_feedback(
    ...
    if let Some(s) = bucket.stats.get_mut(option) {
        ...
        if config.window.enabled {
            ...
            while s.window.len() > config.window.size {
                s.window.pop_front();
            }
        }

        if s.change_boost_remaining > 0 {
            s.change_boost_remaining -= 1;
        }
        ...
        if !config.delayed_feedback.enabled {
            ...
            s.posterior_var =
                (s.reward_variance_recent() / (s.window.len().max(1) as f64)).max(1e-4);
        }
    }
...
fn update_option_states(
    ...
    if option >= bucket.option_states.len() {
        return;
    }
...
fn ensure_option_states_match_algorithm(bucket: &mut ContextBucket, config: &LearningConfig) {
    ...
    let n = bucket.option_states.len();
    bucket.option_states = (0..n)
        .map(|i| match target_kind {
            "betaBernoulli" => OptionState::beta(1.0, 1.0),
            "ucb" => OptionState::ucb_initial(),
            _ => OptionState::weighted(
                bucket
                    .weights
                    .get(i)
                    .copied()
                    .unwrap_or(1.0 / n.max(1) as f64),
            ),
        })
        .collect();
}
...
pub fn apply_feedback_signal(
    ...
    let effective_reward = bucket
        .stats
        .get(option)
        .map(|s| s.posterior_mean)
        .unwrap_or(clipped);
