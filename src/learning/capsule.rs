...
impl CapsuleMemory {
    pub fn to_json(&self) -> serde_json::Value {
        ...
        for (nid, sm) in &self.strategies {
            ...
            let discrete_ood_json = sm
                .discrete_ood
                .as_ref()
                .and_then(|d| serde_json::to_value(d).ok());
            let feature_ood_json = sm
                .feature_ood
                .as_ref()
                .and_then(|d| serde_json::to_value(d).ok());
            strats.insert(
    ...
    pub fn from_json(j: &serde_json::Value) -> Self {
            ...
            for (nid_str, sm_json) in strats {
                ...
                if let Some(cc_map) = sm_json.get("candidateContexts").and_then(|v| v.as_object()) {
                    for (combined_key, bucket_json) in cc_map {
                        let Some((cid_str, ctx_key)) = combined_key.split_once('|') else {
                            continue;
                        };
                        let Some(candidate) = candidate_id_from_str(cid_str) else {
                            continue;
                        };
                        candidate_contexts
                            .insert((candidate, ctx_key.to_string()), parse_bucket(bucket_json));
                    }
                }
                ...
                let discrete_ood = sm_json.get("discreteOod").and_then(|v| {
                    serde_json::from_value::<crate::ood::DiscreteOodDetector>(v.clone()).ok()
                });
                let feature_ood = sm_json.get("featureOod").and_then(|v| {
                    serde_json::from_value::<crate::ood::FeatureOodDetector>(v.clone()).ok()
                });
        ...
        if let Some(ss_json) = j.get("sharedState") {
            if !ss_json.is_null() {
                if let Ok(ss) =
                    crate::shared_state_strategy::SharedStateOptionStrategy::from_json(ss_json)
                {
    ...
    pub fn get_or_init_context(
        ...
        sm.contexts
            ...
            .or_insert_with(|| {
                ...
                let option_states: Vec<OptionState> =
    ...
    pub fn get_or_init_meta_bandit(
        ...
        sm.meta_bandit
            .get_or_insert_with(|| MetaBandit::new_with_candidates(&cands))
    }

    pub fn meta_bandit_for(&self, node_id: u32) -> Option<&MetaBandit> {
        self.strategies
            .get(&node_id)
            .and_then(|sm| sm.meta_bandit.as_ref())
    }
    ...
    pub fn get_or_init_discrete_ood(
        &mut self,
        node_id: u32,
    ) -> &mut crate::ood::DiscreteOodDetector {
        let sm = self
            .strategies
            .entry(node_id)
            .or_insert_with(|| StrategyMemory {
        ...
        sm.discrete_ood
            .get_or_insert_with(crate::ood::DiscreteOodDetector::new)
    }

    pub fn get_or_init_feature_ood(
        &mut self,
        node_id: u32,
        d: usize,
    ) -> &mut crate::ood::FeatureOodDetector {
        let sm = self
            .strategies
            .entry(node_id)
            .or_insert_with(|| StrategyMemory {
    ...
    pub fn discrete_ood_for(&self, node_id: u32) -> Option<&crate::ood::DiscreteOodDetector> {
        self.strategies
            .get(&node_id)
            .and_then(|sm| sm.discrete_ood.as_ref())
    }

    pub fn feature_ood_for(&self, node_id: u32) -> Option<&crate::ood::FeatureOodDetector> {
        self.strategies
            .get(&node_id)
            .and_then(|sm| sm.feature_ood.as_ref())
    }

    pub fn reset_ood_detectors(&mut self, node_id: u32) {
        if let Some(sm) = self.strategies.get_mut(&node_id) {
            if let Some(d) = sm.discrete_ood.as_mut() {
                d.reset();
            }
            if let Some(d) = sm.feature_ood.as_mut() {
                d.reset();
            }
        }
    }
...
fn make_option_states_for_candidate(candidate: CandidateId, weights: &[f64]) -> Vec<OptionState> {
    match candidate {
        CandidateId::Thompson => weights
            .iter()
            .map(|_| OptionState::beta(1.0, 1.0))
            .collect(),
...
fn parse_adwin(j: &serde_json::Value) -> Option<AdwinDetector> {
    ...
    Some(AdwinDetector::restore_state(
        window,
        delta,
        max_size,
        min_subwindow,
...
fn parse_meta_bandit(j: &serde_json::Value) -> Option<MetaBandit> {
    ...
    let ids: Vec<CandidateId> = candidates_json
        .iter()
    ...
    mb.exploration_decay = j
        .get("explorationDecay")
        .and_then(|v| v.as_f64())
        .unwrap_or(5.0);
    mb.min_exploration = j
        .get("minExploration")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.05);
    mb.forgetting_factor = j
...
fn serialize_bucket(bucket: &ContextBucket) -> serde_json::Value {
    let stats_json: Vec<serde_json::Value> = bucket
        .stats
        .iter()
        .map(|s| {
            ...
            if let Some(m) = sj.as_object_mut() {
                ...
                m.insert(
                    "effectiveTries".into(),
                    serde_json::json!(s.effective_tries),
                );
                ...
                m.insert(
                    "changeBoostRemaining".into(),
                    serde_json::json!(s.change_boost_remaining),
                );
        ...
        .collect();
    let option_states_json: Vec<serde_json::Value> = bucket
        .option_states
        .iter()
        .map(OptionState::to_json)
        .collect();
...
fn parse_bucket(bucket_json: &serde_json::Value) -> ContextBucket {
    let weights: Vec<f64> = bucket_json
        .get("weights")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
        .unwrap_or_default();
    let stats: Vec<OptionStats> = bucket_json
        .get("stats")
        .and_then(|v| v.as_array())
        .map(|a| a.iter().map(OptionStats::from_json).collect())
        .unwrap_or_default();
    let updated_at = bucket_json
        .get("updatedAt")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let conformity_calibrator = if let Some(cal_json) = bucket_json.get("conformityCalibrator") {
        parse_conformal(cal_json)
            .unwrap_or_else(crate::conformal::ConformalCalibrator::default_config)
    } else {
        // Legacy path: read raw residuals array left by v5 sidecars.
        let residuals: Vec<f64> = bucket_json
            .get("conformityScores")
            .and_then(|v| v.as_array())
            .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
            .unwrap_or_default();
        crate::conformal::ConformalCalibrator::restore_state(residuals, 500, 30)
    };
    let option_states: Vec<OptionState> = bucket_json
        .get("optionStates")
        .and_then(|v| v.as_array())
        .map(|a| {
            a.iter()
                .filter_map(OptionState::from_json)
                .collect::<Vec<_>>()
        })
...
fn parse_conformal(j: &serde_json::Value) -> Option<crate::conformal::ConformalCalibrator> {
    ...
    Some(crate::conformal::ConformalCalibrator::restore_state(
        residuals,
        max_size,
        min_samples,
    ))
