...
impl OptionState {
    pub fn weighted(w: f64) -> Self {
        Self::Weighted { weight: w }
    }
    pub fn beta(alpha: f64, beta: f64) -> Self {
        Self::BetaBernoulli { alpha, beta }
    }
    pub fn ucb_initial() -> Self {
        Self::Ucb {
            tries: 0.0,
            total_reward: 0.0,
        }
    ...
    pub fn linucb_initial(d: usize, lambda: f64) -> Self {
        Self::LinUcb {
            state: LinUcbState::new(d, lambda),
        }
    }
        ...
        match self {
            ...
            Self::Ucb {
                ...
                if *tries < 1e-9 {
                    ...
                    (total_reward / *tries).clamp(0.0, 1.0)
                }
            }
            Self::LinUcb { state } => {
    ...
    pub fn to_json(&self) -> serde_json::Value {
        match self {
            Self::Weighted { weight } => {
                serde_json::json!({ "kind": "weighted", "weight": weight })
            }
    ...
    pub fn from_json(j: &serde_json::Value) -> Option<Self> {
        ...
        match kind {
            ...
            "linucb" => {
                ...
                let a_inv: Vec<Vec<f64>> = serde_json::from_value(j.get("aInv")?.clone()).ok()?;
                let a: Vec<Vec<f64>> = serde_json::from_value(j.get("a")?.clone()).ok()?;
                let b: Vec<f64> = serde_json::from_value(j.get("b")?.clone()).ok()?;
                let since_last_rebuild = j
                    .get("sinceLastRebuild")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0);
                Some(Self::LinUcb {
                    state: LinUcbState {
