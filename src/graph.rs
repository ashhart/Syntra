...
mod tests {
    ...
    /// Header + one node with attacker-chosen operand/weight counts.
    /// Node body layout: id[4] + op[1] + operand_count[4] + weight_count[4].
    fn make_node_with_counts(operand_count: u32, weight_count: u32) -> Vec<u8> {
        let mut buf = make_header(1, 0, 0, 0);
        buf.extend_from_slice(&0u32.to_le_bytes()); // node id
        buf.push(0x01); // OpCode::ConstInt — valid, so parsing reaches the counts
        buf.extend_from_slice(&operand_count.to_le_bytes());
        buf.extend_from_slice(&weight_count.to_le_bytes());
        buf
    }

    #[test]
    fn rejects_node_with_u32_max_operand_count() {
        // Regression: a 33-byte input made from_bytes attempt a 64 GB
        // Vec::with_capacity (u32::MAX operands x 16-byte Operand) before
        // the decode loop could error. Found by cargo-fuzz on the install
        // path; the header-level guards never saw per-node counts.
        let buf = make_node_with_counts(u32::MAX, 0);
        let res = NeuralGraph::from_bytes(&buf);
        assert!(res.is_err());
        assert_bounds_error(&res.unwrap_err());
    }
    ...
    #[test]
    fn rejects_node_with_u32_max_weight_count() {
        let buf = make_node_with_counts(0, u32::MAX);
        let res = NeuralGraph::from_bytes(&buf);
        assert!(res.is_err());
        assert_bounds_error(&res.unwrap_err());
    }
