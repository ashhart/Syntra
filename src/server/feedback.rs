...
fn do_feedback_hierarchical(
    ...
    let dec_id = match json.get("decisionId").and_then(|v| v.as_str()) {
        ...
        None => {
            return json_resp(
                400,
                &err_json("decisionId is required for hierarchical feedback"),
            );
        }
    };
    let event_line = match state
        .store
        .find_decision_in_job(tenant, job, capsule, &dec_id)
    {
        Ok(Some(line)) => line,
    ...
    let path: Vec<usize> = match dec_entry.get("path").and_then(|p| p.as_array()) {
        ...
        None => {
            return json_resp(
                400,
                &err_json("decision event has no `path` — expected a hierarchical decision"),
            );
        }
    };
    if path.is_empty() {
    ...
    let mut hier_state = match state
        .store
        .load_hierarchical_state_in_job(tenant, job, capsule)
    {
        ...
        None => {
            return json_resp(
                500,
                &err_json("no hierarchical_state on disk — /decide must run before /feedback"),
            );
        }
    };
    let per_level_updates =
        hier_state.apply_feedback_with_candidates(&path, &path, &per_level_candidates, reward);
    ...
    let mut warmup_state = state
        .store
        .load_warmup_state_in_job(tenant, job, capsule)
        .unwrap_or_else(|| {
            crate::warmup::WarmupState::with_capsule_delta(
                30,
                learning_cfg.safety.capsule_adwin_delta,
            )
        });
    let _ = warmup_state.record_feedback(reward);
    if let Err(e) = state
        .store
        .save_warmup_state_in_job(tenant, job, capsule, &warmup_state)
    {
...
pub(super) fn do_feedback(
    ...
    let explicit_context_key = json
        .get("contextKey")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let mut context_key = explicit_context_key
        .clone()
    ...
    let (node_id, option) = if let Some(dec_id) = json.get("decisionId").and_then(|v| v.as_str()) {
            ...
            .find_decision_in_job(tenant, job, capsule, dec_id)
        {
            Ok(Some(event_line)) => {
                ...
                match serde_json::from_str::<serde_json::Value>(&event_line) {
                    Ok(ev) => {
                        ...
                        let was_refused =
                            ev.get("refused").and_then(|v| v.as_bool()).unwrap_or(false);
                        if was_refused {
                            state
                                .store
                                .append_audit_in_job(
                        ...
                        match chosen {
                            Some(dec) => {
                                let nid =
                                    dec.get("node_id").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
                                let opt =
                                    dec.get("chosen_option")
                                        .and_then(|v| v.as_u64())
                                        .unwrap_or(0) as usize;
                                chosen_candidate = dec
                                    .get("candidateId")
                                    .and_then(|v| v.as_str())
                                    .and_then(crate::meta_bandit::CandidateId::from_str);
                                feature_vector = dec.get("featureVector").and_then(|v| {
                            ...
                            None => {
                                return json_resp(
                                    400,
                                    &err_json("decision event has no strategy decisions"),
                                );
                            }
                        }
                    }
            ...
            Ok(None) => {
                return json_resp(404, &err_json(&format!("decisionId not found: {dec_id}")));
            }
            Err(e) => return json_resp(500, &err_json(&e)),
        }
    } else {
            ...
            .and_then(|v| v.as_u64())
        {
            ...
            None => {
                return json_resp(
                    400,
                    &err_json("strategyId/nodeId or decisionId is required"),
                );
            }
        };
    ...
    let node = match ng.nodes.get(node_id as usize) {
        ...
        _ => {
            return json_resp(
                400,
                &err_json(&format!("node {node_id} is not a strategy node")),
            );
        }
    };
    ...
    if option >= n_options {
        return json_resp(
            400,
            &err_json(&format!(
                "option {option} out of range ({n_options} options)"
            )),
        );
    }
    ...
    let warmup_transitioned = matches!(
        warmup_outcome,
        crate::warmup::FeedbackOutcome::WarmupComplete { .. }
    );
    let change_detected = matches!(
        warmup_outcome,
        crate::warmup::FeedbackOutcome::ChangeDetected { .. }
    );
    ...
    let clipped_reward = if learning_cfg.safety.reward_clip > 0.0 {
        reward.clamp(
            -learning_cfg.safety.reward_clip,
            learning_cfg.safety.reward_clip,
        )
    } else {
        reward
    ...
    let delta = ((clipped_reward - ng.nodes[node_id as usize].weights[option]) * learning_rate)
    ...
    if !skip_weight_mutation {
        for j in 0..n_options {
            if j == option {
                ...
                ng.nodes[node_id as usize].weights[j] = (ng.nodes[node_id as usize].weights[j]
                    - delta / (n_options - 1) as f64)
                    .clamp(0.01, 0.99);
            }
        }
    ...
    state
        .store
        .save_graph_in_job(tenant, job, capsule, &updated)
        .ok();
    ...
    let candidates_list_fb: Vec<crate::meta_bandit::CandidateId> = match &learning_cfg.context_spec
    {
        ...
        if is_lin_candidate {
                ...
                let r: Result<(), String> = match (chosen_name, feature_vector.as_ref()) {
                    ...
                    (None, _) => Err(format!(
                        "shared-state feedback: option index {} out of range \
                         ({} option_features)",
                        option,
                        learning_cfg.shared_state.option_features.len()
                    )),
                    (Some(_), None) => Err(
                        "shared-state feedback missing feature vector in decision log".to_string(),
                    ),
                };
            ...
            let r = if let Some(sk) = signal_kind {
                if learning_cfg.delayed_feedback.enabled {
                    crate::learning::apply_feedback_signal(
                        bucket,
                        option,
                        reward,
                        sk,
    ...
    let context_change_detected = {
        let detector = memory.get_or_init_context_detector_with_delta(
            node_id,
            &context_key,
            learning_cfg.safety.context_adwin_delta,
        );
        detector.add(reward)
    };
    ...
    state
        .store
        .save_memory_in_job(tenant, job, capsule, &memory)
        .ok();
