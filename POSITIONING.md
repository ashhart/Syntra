...
If anything in `README.md` contradicts what you read below, this
...
- **Shared-state LinUCB.** A capsule can opt into a shared θ over
  ...
  See `docs/capsule-features/shared-state-linucb.md`.

The bandit core is the Phase A–F + G+H + I work. It is real, it is
tested (over 200 unit tests in the Lycan core, 40+ in the Syntra
wrapper), and it runs unchanged.
...
- **Hierarchical bandits.** A capsule can opt into a nested-tree
  ...
  `docs/capsule-features/hierarchical-bandits.md` and
...
The pattern is the same in every demo under `examples/` once you
...
2. **Operational decisions over computed features.** The capsule program
   *computes* its own features from a recent window the caller passes in
   ...
   `/decide`. The three new demos in `examples/predictive-autoscaling/`,
   `examples/anomaly-routing/`, and
   `examples/seasonal-fraud-threshold/` are the worked examples.
3. **Pull-style feature ingestion via the sidecar.** Run
   `sidecar/syntra-ingest` next to Syntra. Configure it with the
...
- `src/capabilities.rs` — the 26-capability registry, sandbox
  enforcement, EWMA, autoscale-recommend kernels
- `docs/lycan/README.md` — the Lycan language and runtime,
  native-capabilities table at the bottom
- `examples/lycan-internals/demo_capability_pack.lycs` — single
  program touching file I/O, JSON, stats, EWMA, autoscale-recommend
- `examples/lycan-internals/demo_autoscaler.lycs` — four
  competing scaling strategies behind a strategy node
- `CHANGELOG.md` — Phase A–F (bandit core, refusal, drift,
