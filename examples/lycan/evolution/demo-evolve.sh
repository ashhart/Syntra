...
LYC="$ROOT/examples/lycan/demo_evolve_target.lyc"
...
"$LYCAN" compile "$ROOT/examples/lycan/demo_evolve_target.lycs" 2>&1 | sed 's/^/   /'
...
"$LYCAN" evolve "$LYC" --proposal "$ROOT/examples/lycan/proposals/good_strategy.json" --dry-run --min-improvement 0 2>&1 | grep -v "^12502500" | sed 's/^/   /'
echo
...
"$LYCAN" evolve "$LYC" --proposal "$ROOT/examples/lycan/proposals/good_strategy.json" --min-improvement 0 2>&1 | grep -v "^12502500" | sed 's/^/   /'
...
"$LYCAN" evolve "$LYC" --proposal "$ROOT/examples/lycan/proposals/wrong_output.json" 2>&1 | grep -v "^12502500" | sed 's/^/   /'
