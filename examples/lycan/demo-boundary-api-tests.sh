...
"$LYCAN" compile "$ROOT/examples/lycan/demo_mars_horizons_api.lycs" >/dev/null
"$LYCAN" compile "$ROOT/examples/lycan/demo_planetary_defense.lycs" >/dev/null
"$LYCAN" compile "$ROOT/examples/lycan/demo_grid_blackout_prevention.lycs" >/dev/null
"$LYCAN" compile "$ROOT/examples/lycan/demo_evolve_target.lycs" >/dev/null
...
install("mission-control", "mars", root / "examples/lycan/demo_mars_horizons_api.lyc")
mission_policy = {
    "allow_stdout": True,
    "allow_stdin": False,
...
install("planetary-defense", "deflect", root / "examples/lycan/demo_planetary_defense.lyc")
...
install("evolution-lab", "sum", root / "examples/lycan/demo_evolve_target.lyc")
good = json.loads((root / "examples/lycan/proposals/good_strategy.json").read_text())
bad = json.loads((root / "examples/lycan/proposals/wrong_output.json").read_text())
...
for job, target in grid_jobs:
    ...
    install(job, "blackout", root / "examples/lycan/demo_grid_blackout_prevention.lyc")
