...
```java
import io.syntra.retry.RetryClient;
import io.syntra.retry.RetryPolicy;
...
```java
import io.syntra.SyntraClient;
import io.syntra.Decision;
...
$JAVAC --release 17 -d out/main \
  src/main/java/io/syntra/*.java \
  src/main/java/io/syntra/retry/*.java \
  src/main/java/io/syntra/examples/*.java

# Compile tests
$JAVAC --release 17 -cp out/main:$JUNIT_JAR -d out/test \
  src/test/java/io/syntra/retry/*.java
