...
- `capsule.yaml` — the CapsuleSpec input. Carries:
  ...
  - `hierarchical_options:` — the nested tree the runtime walks at
    ...
    `src/hierarchical.rs` (module header).
...
cd examples/hierarchical-region-routing
...
assignment, see `src/hierarchical.rs::propagate_reward`). Both
...
## v1 limitations (tracked in `docs/roadmap.md`)
...
- `docs/capsule-features/hierarchical-bandits.md` — concept doc
  and when-to-use / when-not.
- `docs/roadmap.md` — the integration plan that this demo is
  the worked end-to-end example for.
- `src/hierarchical_state.rs::learns_preferred_path_over_200_rounds`
