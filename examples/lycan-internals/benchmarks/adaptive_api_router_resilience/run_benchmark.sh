...
#   - Lycan compiler available (cargo build --release --bin lycan)
#
# Usage:
...
if [[ ! -x "$LYCAN" ]]; then
  ...
  (cd "$ROOT/.." && cargo build --release --bin lycan --quiet 2>/dev/null) || {
    echo "  ERROR: Cannot build Lycan. Build it manually:"
    echo "    cargo build --release --bin lycan   # from the repo root"
    exit 1
  }
fi
...
if ! curl -sf "$SYNTRA_URL/health" >/dev/null 2>&1; then
  ...
  echo "  Start it with: docker compose up -d (repo root)"
  exit 1
fi
echo "    ok"
echo
