...
| random | 262.5ms | 736.9ms | 5148.8ms | 95.06% | 2.95% | 7.53% | $125.83 | 600 | -2.38 |
...
| weighted_static | 410.6ms | 3200.4ms | 5408.1ms | 91.69% | 4.74% | 13.17% | $112.23 | 1198 | -8.60 |
...
- **Posterior-mean coverage (chosen-action):** 84.2% (nominal 90%)
- **Oracle containment:** 38.1% (quality, not calibration)

Headline: coverage stays at nominal precisely when uncertainty matters
most. Bands are slightly conservative in stable regimes — that costs
efficiency but not safety. The asymmetry runs in the direction operational
systems want.

### Per-phase calibration, set width, and band radius

| Phase | Mean width | P90 width | Mean band radius | Posterior-mean coverage | Oracle containment |
|-------|-----------:|----------:|------------------:|------------------------:|-------------------:|
| phase1_normal | 1.00 | 1.0 | 0.038 | 83.2% | 100.0% |
| phase2_degradation | 1.00 | 1.0 | 0.194 | 81.5% | 18.8% |
| phase3_attack | 1.03 | 1.0 | 0.109 | 85.3% | 95.2% |
| phase4_telemetry | 1.00 | 1.0 | 0.023 | 85.2% | 5.9% |
| phase5_recovery | 1.00 | 1.0 | 0.025 | 85.2% | 0.9% |
| phase6_novel | 1.00 | 1.0 | 0.024 | 84.9% | 0.3% |

### Per-seed coverage spread (single-deployment reliability)

| Phase | Mean cov | Min | Max | Std | Range |
|-------|---------:|----:|----:|----:|------:|
| phase1_normal | 83.2% | 80.0% | 86.3% | 0.014 | 6.3% |
| phase2_degradation | 81.5% | 78.1% | 85.4% | 0.016 | 7.3% |
| phase3_attack | 85.3% | 82.2% | 87.7% | 0.013 | 5.5% |
| phase4_telemetry | 85.2% | 83.3% | 88.7% | 0.012 | 5.4% |
| phase5_recovery | 85.2% | 82.1% | 88.7% | 0.016 | 6.6% |
| phase6_novel | 84.9% | 82.2% | 87.8% | 0.015 | 5.6% |

Coverage at calibration is about the long-run rate; the per-seed spread
tells you how trustworthy a single deployment's coverage will be. Tight
clustering near nominal = strong claim; wide spread = the average obscures
real variance across runs.

Implementation notes: conformity scores are residuals from the chosen
action only (selection bias), so the calibration guarantee is for the
chosen-action reward — not independently for each in-set option. This is
not weighted-conformal (Tibshirani et al. 2019). The conformity buffer is
a sliding window of size 100 with no change-triggered flush; coverage gaps
at phase boundaries indicate the exchangeability violation Gibbs & Candès
2021 (ACI) was built to fix. If gaps are small (≤5pp) the cheap fix is
multiplicative band inflation or synthetic-buffer-seeding on change-
detection alarms; ACI is reserved for cases where the cheap fix doesn't
restore coverage.
...
|----------|---------------------|-------------------------------------------------|
| circuit_breaker | 13.63 | 19.56 |
| ewma_latency | 17.98 | 19.06 |
| lowest_current_latency | 3.34 | 22.19 |
| lowest_error_rate | 16.01 | 19.03 |
| oracle | 19.54 | 19.13 |
| random | -2.38 | 26.10 |
| round_robin | -2.42 | 26.12 |
| weighted_static | -8.60 | 29.85 |
...
### Overall Verdict: **FAIL** (7/8 criteria passed)
