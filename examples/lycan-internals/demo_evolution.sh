...
# Compile fresh
$L compile examples/lycan-internals/demo_router.lycs 2>/dev/null
initial_size=$(wc -c < examples/lycan-internals/demo_router.lyc | tr -d ' ')

echo "  FRESH COMPILE:"
$L stats examples/lycan-internals/demo_router.lyc 2>/dev/null | grep -E "Nodes:|Edges:|Binary|Branch|fired|Converged"
echo ""

echo "  Running 10 rounds of traffic simulation..."
for i in $(seq 1 10); do
    $L examples/lycan-internals/demo_router.lyc > /dev/null 2>/dev/null
    printf "  Run %2d complete\n" $i
done
echo ""

final_size=$(wc -c < examples/lycan-internals/demo_router.lyc | tr -d ' ')

echo "  AFTER 10 RUNS:"
$L stats examples/lycan-internals/demo_router.lyc 2>/dev/null | grep -E "Nodes:|Edges:|Binary|Branch|fired|Converged|Specialized"
echo ""
...
$L compile examples/lycan-internals/demo_classifier.lycs 2>/dev/null
initial_size=$(wc -c < examples/lycan-internals/demo_classifier.lyc | tr -d ' ')

echo "  FRESH COMPILE:"
$L stats examples/lycan-internals/demo_classifier.lyc 2>/dev/null | grep -E "Nodes:|Branch|Converged"
echo ""

echo "  Running 10 rounds of transaction processing..."
for i in $(seq 1 10); do
    $L examples/lycan-internals/demo_classifier.lyc > /dev/null 2>/dev/null
    printf "  Run %2d complete\n" $i
done
echo ""

final_size=$(wc -c < examples/lycan-internals/demo_classifier.lyc | tr -d ' ')

echo "  AFTER 10 RUNS:"
$L stats examples/lycan-internals/demo_classifier.lyc 2>/dev/null | grep -E "Nodes:|Binary|Branch|fired|Converged|Specialized"
echo ""
echo "  Binary: $initial_size -> $final_size bytes"
