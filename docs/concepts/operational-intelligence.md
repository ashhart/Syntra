...
worked examples now under `examples/` follow it; you can read
them in any order:
...
metadata block in `src/capabilities.rs` describing its inputs,
...
`src/capabilities.rs`; the
[`docs/lycan/README.md`](../lycan/README.md)
...
`tests/change_detection_characterization.rs`, which sweeps a
...
need adjustment. See `docs/known-issues.md` for the standing
caveat.
