...
`rand_f64()` in `src/learning.rs:2202` uses `SystemTime::now()`
...
lines across `src/learning.rs`, `src/server/mod.rs`, and
...
characterization runs in `tests/change_detection_characterization.rs`
...
`do_report` (`src/server/inspect.rs`) now surfaces three previously-
