...
`docs/roadmap.md` for the v1 limitations that carry forward
...
`src/hierarchical.rs`, plus the CapsuleSpec-level rules in
`src/capsule_spec.rs::validate_hierarchical`):
...
See [`examples/hierarchical-region-routing/`](../../examples/hierarchical-region-routing/)
...
[`src/hierarchical_state.rs`](../../src/hierarchical_state.rs)
