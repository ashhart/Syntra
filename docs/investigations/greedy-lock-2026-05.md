...
- `(strategy ...)` is for **Lycan-standalone** programs that converge
  ...
  Useful for the strategy-learning demos under `examples/lycan/` that
...
2. The /feedback we sent was actually applied to the node's weights
   (via `do_feedback` in `src/server/feedback.rs`, which handles
...
Syntra's capsule_compiler (src/capsule_compiler.rs:89) explicitly
...
- **Path B (rejected)**: change `OpCode::Strategy` to defer to
  ...
  programs under `examples/lycan/` that rely on the time-based
...
1. `examples/predictive-autoscaling/program.lycs` — `(strategy ...)` → `(choice ...)`.
2. `examples/anomaly-routing/program.lycs` — same.
3. `examples/seasonal-fraud-threshold/program.lycs` — same.
4. `docs/lycan/language/strategy-nodes.md` — added a "When to use which"
   section near the top distinguishing the two forms.
5. `src/capsule_compiler.rs` (or wherever `syntra author` lives)
...
6. Three demo READMEs — "What to expect" sections rewritten with
   ...
   trajectory above.
7. `CHANGELOG.md` — Phase I followup entry covering this fix.
