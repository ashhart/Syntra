...
- `docs/capsule-features/hierarchical-bandits.md` — concept
...
1. **`src/capsule_spec.rs`** — add optional field around line 50:
   ```rust
...
2. **`src/capsule_compiler.rs`** — in `compile_to_dir`
...
3. **`src/server/decide.rs` `do_decide`**: after
...
4. **`src/server/feedback.rs` `do_feedback`**: after
...
5. **`src/store.rs`** (around line 447, the memory-sidecar
