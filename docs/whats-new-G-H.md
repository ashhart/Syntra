...
time you read this, the source of truth is the `src/` files and this
...
set of metric families emitted by `render_metrics` in `src/server.rs`:
...
Three scopes are defined in `src/auth_tokens.rs`:
...
(the `A_inv` and `b` matrices in `src/linucb.rs`) as LinUCB, but at
...
`src/learning.rs::ActionSpace`) allows a capsule's K options to be
...
`src/hierarchical.rs`. The shape allows a capsule's option set to be
...
`src/feature_schema.rs` gained a fourth `FeatureType` variant:
...
`src/linucb.rs` contains `LinUcbSharedState` — a shared-parameter
...
- **Status:** schema and credit-assignment logic exist in
  `src/hierarchical.rs`. The public integration surface
  (`HierarchicalSpec::from_json`, `validate`, `enumerate_paths`,
...
- **Status:** `FeatureType::TimeSeries` is parsed and validated in
  `src/feature_schema.rs`. The `TimeSeriesWindow` type (push, aggregate,
...
- **Status:** `LinUcbSharedState` exists in `src/linucb.rs`. The shared
...
watch the `src/` source tree and the `CHANGELOG.md`. When the server
