...
container image. The build context is the repo root (the single
`syntra` crate).
...
The five flagship capsule sources are baked in from `examples/`
...
# Sync the REPO ROOT here (this artifact's Dockerfile builds from the
# repo-root build context).
# rsync, git clone, scp — whatever you prefer.

cd deploy/try-instance
cp .env.example .env
...
0 0 * * * /opt/syntra-try/deploy/try-instance/reset.sh

# Health probe every five minutes.
*/5 * * * * /opt/syntra-try/deploy/try-instance/monitor.sh
```
...
```bash
cd /opt/syntra-try/deploy/try-instance
...
```bash
# On a build host with the repo checked out:
cd /path/to/syntra
docker build -t ghcr.io/<your-org>/syntra-try:latest \
    -f deploy/try-instance/Dockerfile .
