...
The Dockerfile's build context is the repo root. Sync the whole
tree:
...
#     /path/to/syntra/ root@<VPS_IP>:/opt/syntra-try/
```
...
```bash
ls /opt/syntra-try/
# → Cargo.toml  src  deploy  examples  ...
```
...
```bash
/opt/syntra-try/deploy/try-instance/monitor.sh
...
/opt/syntra-try/deploy/try-instance/reset.sh
